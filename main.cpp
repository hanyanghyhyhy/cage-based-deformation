...
Eigen::MatrixXd headPointsCage, trailPointsCage, headPointsCageDF1, trailPointsCageDF1;
Eigen::MatrixXd mvcWeights;
...
Eigen::MatrixXd computeBarycentricCoordinates(int index,int rows, Eigen::MatrixXi controlFace, double theta0,double theta1,double theta2,double l0,double l1,double l2){
...
Eigen::MatrixXd computeSingleWeight(Eigen::MatrixXd modelVetex, Eigen::MatrixXd controlVetex, Eigen::MatrixXi controlFace){
    ...
    for (int i = 0; i < controlVetex.rows() ; i++) {
        ...
        Eigen::Vector3d d(modelVetex(0)-individualCP(0),modelVetex(1)-individualCP(1),modelVetex(2)-individualCP(2));
...
>>>>>>> 2abfc754b112e612894dd2d5bf65243facc06b7e

        if((M_PI - h) <= EPSILON){
            // x lie on t, use 2D barycentric coordinates
            return computeBarycentricCoordinates(j,controlVetex.rows(),controlFace,theta0,theta1,theta2,l0,l1,l2);
        }
...
int main(int argc, char *argv[]) {
    std::string modelPath = "../example_meshes/model.off";
    std::string cagePath = "../example_meshes/cage.off";
    std::string dfPath1 = "../example_meshes/deformation.off";
    // load the model
<<<<<<< HEAD
    Eigen::MatrixXd V;
    Eigen::MatrixXi F;
...
=======
    TargetModel targetModel = TargetModel(modelPath);
    ControlCage controlCage = ControlCage(cagePath);
    // Calculate the weight in advance to exoedite the computation
    mvcWeights = computeWeight(targetModel.m_V, controlCage.m_V, controlCage.m_F);
>>>>>>> 2abfc754b112e612894dd2d5bf65243facc06b7e
    ...
    {
        // Add new group
        if (ImGui::CollapsingHeader("Cage-Based Deformation", ImGuiTreeNodeFlags_DefaultOpen))
        {
            // Add buttons
            if (ImGui::Button("Reset Model", ImVec2(-1,0))){
                viewer.data().clear();
                TargetModel targetModel = TargetModel(modelPath);
                viewer.data().set_mesh(targetModel.m_V, targetModel.m_F);
                viewer.core.align_camera_center(targetModel.m_V,targetModel.m_F);
            }
            if (ImGui::Button("Show Control Cage", ImVec2(-1, 0))) {

                controlCage.loadCage(headPointsCage, trailPointsCage);
        ...
        if (ImGui::CollapsingHeader("Deformation 1", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            if (ImGui::Button("Show Deformed Cage", ImVec2(-1,0))){
                ...
                deformedCage1.loadCage(headPointsCageDF1, trailPointsCageDF1);
                viewer.data().add_edges(headPointsCageDF1, trailPointsCageDF1, Eigen::RowVector3d(1, 0.5, 0.5));
            }
            if (ImGui::Button("Deform Model", ImVec2(-1,0))){
                ControlCage deformedCage1 = ControlCage(dfPath1);
                deformedCage1.loadCage(headPointsCageDF1, trailPointsCageDF1);
                Eigen::MatrixXd updated_Vertex = mvcWeights*deformedCage1.m_V;
                viewer.data().set_mesh(updated_Vertex, targetModel.m_F);
                viewer.core.align_camera_center(updated_Vertex,targetModel.m_F);
            }
